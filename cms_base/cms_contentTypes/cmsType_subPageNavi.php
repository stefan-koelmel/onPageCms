...
class cmsType_subPageNavi_base extends cmsType_contentTypes_base {
    ...
    function show($contentData,$frameWidth) {
        ...
        switch ($navType) {
            ...
            case "scrollTop" :
                $this->show_scrollTop($contentData,$frameWidth);
                break;

            case "scrollList" :
                $this->show_scrollList($contentData,$frameWidth);
                break;
        }
    }
...
//        foreach ($pageData as $key => $value ) echo ("$key = $value <br>");
        ...
        if (is_array($pageList) AND count($pageList)) {
            ...
            // for ($i=0;$i<count($pageList);$i++) {
                ...
                if (count($subData) and $class=='index' AND $sessionName) {
                    // echo ("SUBDATA FOR $subPageData[name] - Ses = $sessionName ".$_SESSION[$sessionName]."<br>");
                ...
                if ($show) {
                   ...
                   // echo ("<span class='subPageNavi_title' style=''>$showName</span><br />");
                    ...
                    if ($useInfo) {
                        $showText = cms_text_getLg($subPageData[description]);
                        if (!$showText) {
                            $showText = cms_text_getLg($subPageData[pageTitle]);
                        }
    ...
    function show_scrollTop($contentData,$frameWidth) {
        echo ("<div class='mainJavaButton cmsScroll'>Seitenanfang</div>");
    }

    function show_scrollList($contentData,$frameWidth) {
       ...
       //  echo ("<h1>Scroll List $pageName</h1>");
        ...
        foreach ($myScroll as $id => $contData) {
            ...
            if ($show) {
                ...
                echo ("<div id='anker_".$contData[id]."' class='mainJavaButton cmsScroll' >$title</div>");
            }
        }
    }
        ...
        switch ($type) {
            ...
            case "index" :
                ...
                $res[] = $addData;
                 ...
                 $addData = array();
                $addData["text"] = "Nur in Navigation";
           ...
           case "parallel" :
            ...
            case "scrollList" :
                ...
                if (is_array($setTitle)) {
                    $oldTitleList = $_POST[oldTitle];
                    // show_array($oldTitleList);
                    
                    foreach ($setTitle as $contId => $newTitle ) {
                        $oldTitle = $oldTitleList[$contId];
                        if ($oldTitle != $newTitle AND $newTitle) {
                            echo ("<b> RENAME </b>");
                            echo ("id = $contId neuer Titel '$newTitle' alter Titel='$oldTitle' <br>");
                           
                            $saveData = array("title"=>$newTitle);
                            cms_content_save($contId,$saveData);
                            
                            $myScroll["inh_".$contId][title] = $newTitle;
                        }                       
                ...
                foreach ($myScroll as $id => $contData) {
                    ...
                    $addData = array();
                    $addData["text"] = "Inhalt type=$contType";
