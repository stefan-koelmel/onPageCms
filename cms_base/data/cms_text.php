...
function cms_text_adminLg() {
    $adminLg = $_SESSION[adminLanguage];
    if ($adminLg) return $adminLg;
    
    echo ("<h1>SET ADMIN LG TO ENGLISCH </h1>");
    $adminLg = "en";
    $_SESSION[adminLanguage] = $adminLg;
...
function cms_text_getSettings() {
    ...
    for ($i=0;$i<count($help);$i++) {
        list($code,$enabled,$editable,$active) = explode(":",$help[$i]);
...
function cms_text_checkLanguages($lgList) {
    $query = "SELECT * FROM `".$GLOBALS[cmsName]."_cms_text` WHERE `id` = 1 ";
    $result = mysql_query($query);
    if (!$result) return "error";
    
    $lgData = mysql_fetch_assoc($result);
    if (!is_array($lgData)) return "noData for Languages";
                    ...
                    default :
                        switch ($key) {
                            case "search" : 
                                ...
                                $filterQuery .= "`lg_dt` LIKE '%$value%' AND `name` = 'headline' " ;
                                break;
                                
                            case "searchText" : 
                                if ($filterQuery != "") $filterQuery .= " AND ";
                                $filterQuery .= "`lg_dt` LIKE '%$value%' AND (`name`='headline' OR `name`='text')" ;
                                break;
