...
class cmsAdmin_user_base extends cmsAdmin_editClass_base {
    ...
    function show_list() {
        $this->userList($this->frameWidth);
    }
    
    function show_new() {
        $projectId = 0;
        $this->userEdit($projectId,$this->frameWidth); 
    }
    ...
    function userList($frameWidth) {
        ...
        $sort = $_GET[sort];
        $filter = array();
        $filter[userLevel] = "<=".$this->userLevel;
        ...
        $this->admin_show_list($userList);
        return 0;
        
        $showList = $this->user_showList();
    ...
    function admin_get_filterList_own() {
        ...
        $filterList[userLevel]["showData"] = array("maxLevel"=>$this->userLevel,"submit"=>1,"empty"=>"Alle Benutzerebenen");
