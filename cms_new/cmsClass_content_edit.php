...
class cmsClass_content_edit extends cmsClass_content_editType {
    function init_edit() {
        ...
        $this->showMode = $this->session_get(editMode); // $_SESSION[editMode];

        $this->minEditWidth = 500;
        
        if ($this->mobileEnabled) {
            if ($this->targetData[target]!="pc") {
                // echo ("remove minEditWidth because MOBIL  ".$this->targetData[width]." ".$this->pageClass->pageWidth."  <br>");
                $this->minEditWidth = $this->pageClass->pageWidth;
            }
        }
        ...
        if ($this->pageData[dynamic]) {
            ...
            $dynamic1_source = $this->pageData[data][dataSource];
            if ($dynamic1_source) {
                if ($_GET[$dynamic1_source]) {
                    // echo ("Dynamic GET for $dynamic1 = $_GET[$dynamic1] <br>");
                    $goPage .= "?".$dynamic1_source."=".$_GET[$dynamic1_source];
                }
            }
            $dynamic2 = $this->pageData[data][dynamic2];
            // echo ($dynamic2." | ");
            if ($dynamic2) {
                $dynamic2_source = $this->pageData[data][dataSource2];
                // echo ("source = $dynamic2_source ");
                if ($dynamic2_source) {
                    // echo ("DYNAMIC 2 SOURCE = $dynamic2_source <br>");
                    if ($_GET[$dynamic2_source]) {
                        if (strpos($goPage,"?")) $goPage .= "&";
                        else $goPage .= "?";
                        $goPage .= $dynamic2_source."=".$_GET[$dynamic2_source];
                    }
                }
                
            }
            
            // foreach ($this->pageData[data] as $key => $value ) echo ("$key =>  $value <br>");
        }
    ...
    function edit_content_show() {
        ...
        if ($setWidth < $this->minEditWidth) {
            $setWidth = $this->minEditWidth;
            $leftPos = $this->contentData[leftPos];
            $pageWidth = $this->pageClass->pageWidth;
            $marginLeft = 0;
            if (($leftPos + $setWidth) > $pageWidth) $marginLeft = $pageWidth - $leftPos - $setWidth;
           // foreach ($this->contentData as $key => $value) echo ("$key => $value <br>");
            // echo ("SET SETWidth to $setWidth LeftPos = $leftPos  $pageWidth ML=$marginLeft <br>");
            $setWidth = $setWidth - 2;
    ...
    function edit_show_tab_input() {
        ...
        $userLevel = $this->session_get(userLevel);
        $border = 1;
        $padding = 3;
        $leftWidth = 200;

        $useWidth = $this->frameWidth;
        if ($useWidth < $this->minEditWidth) $useWidth = $this->minEditWidth;
        $rightWidth = $useWidth - $leftWidth - (2 * ($padding + $border));
        ...
        if ($sameHeight) {
            $mainDiv = "cmsSameHeight";
            if ($mainDiv) $out .= div_start_str ($mainDiv);
        }
        
        foreach ($this->inputList as $key => $tabList) {
            // SORTABLE
            
            $sortable = 0;
            foreach ($tabList as $nr => $v) {
                if (!is_array($v)) continue;
                if ($v[sort]) {
                    $sortable = 1;
                    // $out .= "SORT is on in $key for $nr = $v[text]<br>";
                }
            }
            ...
            if ($sameHeight) $divName .= " cmsSameHeightItem";
            if ($key != $this->selectEdit) $divName .= " cmsEditFrameHidden";
            $divData = array(); //"editName"=>$key);
            $divData[id] = "cmsEditTabFrame_".$key;
            if ($sortable) $divName .= " cmsEditFrame_sortable";
            ...
            $start = 0;
            if ($tabList[showTab]) {
                unset($tabList[showTab]);

            }
            if ($tabList[showName]) {
                unset($tabList[showName]);
            }
            
            foreach ($tabList as $nr => $item ) {
                
               
                if (is_array($item) AND is_string($nr)) {
                    $out .= "$nr = $item ".count($item)."<br>";
                    foreach ($item as $k => $v) $out .= " --> $k => $v <br>";
                } 
                if (is_array($item)) {
                    $showInput = 1;
                    $showMode = $item["mode"];
                    ...
                    $itemSort     = $item[sort];
                    if ($itemSort) $lineDivName .= " inputLine_sortable";
                    ...
                    if (is_array($item[div])) {
                        $divName2 = $item[div][divname];
                        $divStyle = $item[div][style];
                        $divContent = $item[div][content];
                        $divData2 = array();
                        foreach ($item[div] as $divKey => $data) {
                            switch ($divKey) {
                        ...
                        if ($itemSort) {
                            $editText .= "<div class='inputLine_dragButton' style='display:inline-block;'>";
                            $editText .= "<img border='0px' src='/cms_$this->cmsVersion/cmsImages/cmsMove.png'>";
                            $editText .= "</div>";                        
                        }
                        ...
                        if (is_array($item[input])) { 
                            $editInput = "";
                            foreach ($item[input] as $inputNr => $inputItem ) {
                                if (is_string($inputItem)) {
                                    $editInput .= $inputItem;
                                    continue;
                                }    
                                $mode = $inputItem[mode];
                                switch ($mode) {
                                    ...
                                    case "toggleItem" : 
                                        $toggleId = $inputItem[id];
                                        $state = $inputItem[state];
                                        $toggleClass = "inputToggleItem";
                                        if (!$state) $toggleClass .= " inputToggleItem_hidden";
                                        $editInput .= "<div class='$toggleClass' id='$toggleId' >";
                                        break;
                                        
                                    default : $editInput .= "m[$mode]"; break;                                
                                }
                                $editInput .= $inputItem[input];
                                switch ($mode) {
                                    case "toggleItem" :
                                        $editInput .= "</div>";
                                        break;
    ...
    //                        $out .= $div3;
                    ...
                    switch ($item) {
                        ...
                        default:
                            if ($item) {
                                $out .= "unkown addInput in $nr Key($key) = $item<br />";
                            } else {
                                $out .= "<h1>NO IITEM fo $key </h1>";
                            }                          
                    }
                }
            }
        ...
        if ($mainDiv) $out .= div_end_str($mainDiv);
...
//                        }
            ...
            foreach($addInput as $key => $value) {
                 if (is_integer($key)) {
                    ...
                    if (!is_array($value)) {
                        ...
                        // echo ("addInput is Asso-Array $key = $value anz=".count($tabInput[$key])."<br />");
                        // show_array($value);
                        foreach($value as $key2 => $value2) {
                            if (is_integer($key2)) {
                               //  echo ("addInput is Integer $key2 = $value2 to target $key <br />");
                                ...
                                // echo ("NO INTEGER addInput is Integer $key2 = $value2 to target $key <br />");
                                switch ($key2) {
                                    case "showTab" :
                                        if (!is_array($tabInput[$key])) $tabInput[$key] = array();
                                    ...
                                    default :
                                        // echo ("<h1>addInput is Asso-Array $key / $key2 </h1><br />");
                        ...
                        // echo ("addInput AFTER Asso-Array anz=".count($tabInput[$key])."<br />");
                    }
                }
            }
        }
    ...
    function edit_post_save($close) {
        ...
        if (is_array($editContent)) {
            ...
            $specialView = $this->contentData[specialView];
            // echo ("HIER $specialView <br>");
            $specialSave = 0;
            if ($specialView) {
                switch ($specialView) {
                    case "contentName" :
                        $specialId = $this->contentData[specialId];
                        if ($specialId) {
                            echo ("SAVE CONENTNAME with $specialId <br />");
                            $contentSaveResult = $this->content_save_contentName($editContent,$doSave);
                            $specialSave = 1;
                        }
                        break;
                    default :
                        echo "unkown SpecialView($specialView) in edit_post_save()<br>";                        
                }
            ...
            // echo ("$contentError out = $contentOut <br>");
    ...
    function content_save_contentName($editContent,$doSave) {
        echo ("content_save_contentName $editContent $doSave <br> ");
        
        $orgData = $this->contentData[specialOrg];
        // echo ("ORG VALUE = $orgData <br>");
        $specialData = $this->contentData[specialData];
        $saveData = array();
        foreach ($editContent as $key => $value) {
            if (is_array($value)) {
                if (!is_array($saveData[$key])) $saveData[$key] == array();
                foreach ($value as $dataKey => $dataValue) {
                    if (is_array($dataValue)) {
                        echo ("DATA VALUE $key | $dataKey is array! <br>");
                    } else {
                        if (!is_array($orgData[$key])) {
                            $help = str2Array($orgData[$key]);
                            if (is_array($help)) $orgData[$key] = $help;
                        }
                        $orgValue = $orgData[$key][$dataKey];
                       

                        if ($orgValue == $dataValue) {
                            // echo ("no Change in data [$key] for key($dataKey) ($dataValue == $orgValue) <br>");
                        } else {
                            echo ("Change in data [$key] for key($dataKey) from $orgValue to <b>$dataValue</b> <br>");
                            $saveData[$key][$dataKey] = $dataValue;
                        }
                    }
                }
                ...
                $orgValue = $orgData[$key];
                if ($orgValue == $value) {
                    // echo ("no Change for $key ($value == $orgValue) <br>");
                } else {
                    echo ("Change for key($key) from $orgValue to <b>$value</b> <br>");
                    $saveData[$key] = $value;
                }
            }
        }
        ...
        foreach ($saveData as $key => $value ) {
            if (is_array($value)) {
                echo ("<k> SAVE in Array </k><b> $key </b><br />");
                foreach ($value as $dataKey => $dataValue) {
        ...
        $contentSaveResult = $this->content_save($saveData,$doSave);
        
        
//        $contentSaveResult = array();
//        $contentSaveResult[error] = 1; 
//        $contentSaveResult[change] = 1;
//        $contentSaveResult[out] = "Trulla";
//        $contentSaveResult[value] = $editContent;
    ...
    function content_save($editContent,$doSave=0) {
        ...
        foreach ($editContent as $key => $value) {
            switch($key) {
                case "layout" : break;
                case "oldType" : break;
                case "specialView" : break;
                case "specialId" : break;
                case "specialData" : break;
...
//        $specialType = $this->contentData[specialType];
//        if ($specialType) {
//            
//            switch ($specialType) {
//                case "contentName" :
//                    $contentNameId = $this->contentData[specialId];
//                    $contentNameData = $this->contentData[specialData];
//                    if (!$contentNameId) {
//                        echo ("no ContentId for contentName $specialType <br>");
//                        return 1;
//                    }
//                    echo ("SPECIAL EDIT contentName $contentNameId $contentNameData <br>");
//                    break;
//                default : 
//                    echo ("<h1>Edit SpecialType $specialType </h1>");
//            }
//        }
        ...
        if ($this->doEdit) {
            if ($active) {
                ...
                echo ("<img src='/cms_".$this->cmsVersion."/cmsImages/cmsEdit.png' border='0px'>");
                echo ("</div>");

                // edit verschieben
                echo ("<div class='dragButton' style='display:inline-block;'><img src='/cms_".$this->cmsVersion."/cmsImages/cmsMove.png' border='0px'></div>");

                // edit Löschen
                $goPage = $this->goPage;
                $goPage .= "?editMode=deleteContent&editId=$this->contentId;#editFrame_$this->contentId";
                // echo (" <a href='$goPage'><img src='/cms_".$this->cmsVersion."/cmsImages/cmsDelete.png' border='0px'></a>");
...
//        
        ...
        echo ("<img src='/cms_".$this->cmsVersion."/cmsImages/cmsDelete.png' border='0px'>");
        echo ("</div>");
    ...
    function edit_head_Sitebar() {
        ...
        // Roll Image
        // echo ("<img src='/cms_".$this->cmsVersion."/cmsImages/cmsEditClose.png' border='0px'>");

        // edit Edit
        echo ("<div class='cmsContentFrame_editButton' id='editButtonID_$this->contentId' style='display:inline-block;'><img src='/cms_".$this->cmsVersion."/cmsImages/cmsEdit.png' border='0px'></div>");
        ...
        echo ("</div>");
        // edit Cut
        // echo (" <a href='$goPage'><img src='/cms_".$this->cmsVersion."/cmsImages/cmsCut.png' border='0px'></a>");
    ...
    function editContent_settings() {
        ...
        $specialView = $this->contentData[specialView];
        // foreach ($this->contentData as $key => $value) echo ("cD $key => $value <br>");
        
        $contentId = $this->contentId;
        $contentType = $this->contentType;
        if ($specialView) switch($specialView) {
            case "contentName" :
                $contentNameId   = $this->contentData[specialId];
                $contentNameData = $this->contentData[specialData];
                
                if ($contentNameId) {
                    $contentId = $contentNameId;
                    $contentType = "contentName";
                    $showContentName = 0;
                
                    echo ("<h2> contentName edit $contentNameId $contentNameData </h2>");
                }
                break;
        }
        ...
        $addData["input"] = "<input type='text' readonly='readonly' name='editContent[id]' value='$contentId' />";
        $addData["mode"] = "Admin";
        $settings[] = $addData;
        ...
        if ($showType) {    // Typ
            ...
            if ($layoutName OR $_GET[editLayout]) {
                 $input = cms_contentLayout_selectType($contentType,"editContent[type]","button");
            } else {
                $showData = array("viewMode"=>"select","width"=>190,"id"=>"selectContentType");
                $input = $this->editContent_SelectSettings("contentType",$contentType,"editContent[type]",$showData);
            }
            
            if ($specialView == "contentName") {
                $input .= "contentNameType = '$this->contentType' ";
            }
            $addData[input] = $input;
            $addData["mode"] = "More";
            $settings[] = $addData;
        ...
        if ($specialView == "contentName") {
            $addData = array();
            $addData["text"] = $this->lga("content","frameText_contentSelect");
            $contentSelect = $this->editContent[data][contentName];
            $addData["input"] = cms_content_Select_contentName($contentSelect,"editContent[data][contentName]");
            $addData["mode"] = "Simple";
        ...
        if ($this->mobileEnabled) {
            $addData = array();
            $addData["text"] = $this->lga("content","frameText_mobilShow",":");//"Anzeigen ab";
            $showData = array("mode"=>"simpleShow");
            $addData["input"] = $this->editContent_selectSettings("mobileShow",$this->editContent[data][mobileShow], "editContent[data][mobileShow]", $showData); 
            $addData["mode"] = "More";
            $settings[] = $addData;
        }
        ...
        if ($showLevel) { // UserLevel
            ...
            $addData["input"] = cms_user_selectLevel($this->editContent[showLevel],$userLevel,"editContent[showLevel]");
            $addData["mode"] = "More";
            $settings[] = $addData;
            ...
            if ($this->editContent[showLevel] == 3) {
                ...
                $addData["input"] = cmsUser_selectUserList($data,$userLevel,"editContent[data]");
                $addData["mode"] = "More";
                $settings[] = $addData;
            }
        }
        ...
        if ($showToLevel) { // UserLevel
            ...
            $addData["input"] = cms_user_selectLevel($this->editContent[toLevel],$userLevel,"editContent[toLevel]");
            $addData["mode"] = "More";
            $settings[] = $addData;
        }
...
//        }
    ...
    function editContent_systemFrame() {
        ...
        $systemList = cmsSystemFrame_editList($this->editContent);
        return $systemList;
    }
    ...
    function editContent_wireframeSettings() {
        ...
        if (!$this->wireframeEnabled) return 0;
        
        
        $data = $this->editContent[data];
        if (!is_array($data)) $data = array();

        $wireframeData = $this->editContent[wireframe];
        if (!is_array($wireframeData)) $wireframeData = array();
        
        $wireOn = $data[wireframe];
        ...
        $wireList = array();
        $wireList[showTab] = "More";
        $wireList[showName] = $this->lga("content","frameText_tabWireframe");//"Wireframe-Optionen";
        

        $add = array();
        $add[text] = $this->lga("content","wireframe_wireframeOnText",":");//"Wireframe Optionen an";
        ...
        $editList = array();
        $editList[headLine] = "Über"; //$this->lga("content","wireframe_headlineText",":");
        $editList[subHeadLine] = "SubHead";
        $editList[text] = "Text";
        $editList[image] = "Bild";
        $editList[frameHeadLine] = "Rahmen Überschrift";
        $editList[frameHeadText] = "Rahmen Text Oben";
        $editList[frameSubText] = "Rahmen Text Unten";
        
        if ($this->mobileEnabled) {
            $mobileShowData = array();
            $mobileShowData[mode] = "wireframe";
            $mobileShowData[viewMode] = "select";
        }
        
        foreach ($editList as $key => $name) {
            $add = array();
          
            $add[text] = $this->lga("content","wireframe_".$key."Text",":");//"Überschrift";
            $isOn = $wireframeData[$key];
            if ($isOn) $checked = "checked='checked'"; else $checked = "";
            $input =  "<input class='editWireframe_option' type='checkbox' value='1' name='editContent[wireframe][$key]' $disabled $checked />";

            // Get LANGEUAGE INPUT Titel
            $showData=array("width"=>200,"out"=>"input");
            $input .= $this->editContent_languageString($wireframeData[$key."Text"],"editContent[wireframe][".$key."Text]",$showData);

            if ($this->mobileEnabled) {
                $selectValue = $wireframeData[$key."Mobile"];
                $formName = "editContent[wireframe][".$key."Mobile]";
                
                $input .= " Mobil: ".$this->editContent_selectSettings("mobileShow", $selectValue, $formName, $mobileShowData);
            }
        ...
        // foreach ($wireframeData as $key => $value) echo ("$key = $value <br>");
        return $wireList;
