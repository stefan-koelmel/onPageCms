...
//        }
    ...
    $(".cmsEditFrame_sortable").sortable({
        handle: '.inputLine_dragButton',
        cursor:"move",
        stop: function(event) {
            inputId = $(this).attr("id");
            // sortInputItems(inputId);
            i=0;
            $("#"+inputId).children(".inputLine_sortable").each(function(){
                i=i+1;
            
                $(this).children(".inputRight").children(".inputLineSortNr").val(i);
                // $(this).children(".inputRight").css("background-color","#fc0");
            })
            
        }
    })
    $(".cmsEditFrame_sortable").disableSelection();
    
    $(".adminFrame_sortable").sortable({
        handle: '.inputLine_dragButton',
        cursor:"move",
        stop: function(event) {
            inputId = $(this).attr("id");
            ...
            // sortInputItems(inputId);
            i=0;
            $("#"+inputId).children(".adminFrame_sortItem").each(function(){
                i=i+1;
            
                $(this).children(".adminFrame_sortValue").val(i);
                // $(this).children(".inputRight").css("background-color","#fc0");
            })
            
        }
    })
    $(".adminFrame_sortable").disableSelection();
...
$(".cmsSelectEditMode").live("click",function(){
   ...
   equalHeight($(".cmsSameHeight").children(".cmsSameHeightItem"));
...
$(".cmsSelectTarget").live("click",function(){
    ...
    if (setTarget == "mobil") {
        ...
        setWidth = 480;
        if ($("#setDirection_portrait").hasClass("cmsSelectDirectionSelected")) setWidth = 320;
        
        setFrameSize(setWidth);
        ...
        setFrameSize(pageWidth);
    }
    
    url = "/cms_"+cmsVersion+"/getData/setSession.php";
    url += "?target_target="+setTarget.toLowerCase();
    url += "&cmsName="+cmsName;
    url += "&cmsName="+cmsName;
    $.get(url,function(res){
...
//    
    ...
    url = "/cms_"+cmsVersion+"/getData/setSession.php";
    url += "?target_orientation="+setDirection.toLowerCase();
...
$("#setUserShowLevel").live("change",function(){
    ...
    val = $(this).val();
    url = "/cms_"+cmsVersion+"/getData/setSession.php";
    url += "?showLevel="+val+"&pageList=0&pageGroupList=0";
    url += "&cmsName="+cmsName;
    $.get(url,function(res){
        window.location.reload();
    })
...
$(".cmsSitemap_reset").live("click",function(){
    url = "/cms_"+cmsVersion+"/getData/setSession.php";
    url += "?pageList=0&pageGroupList=0";
    url += "&cmsName="+cmsName;
    $.get(url,function(res){
        window.location.reload();    
    })
...
$(".inputToggleSelect").change(function(){
    id = $(this).attr("id");
    id = id.substr(13);
    val = ($(this).attr("checked") == "checked");
    
    
    toggleItem = "toggleItem_"+id;
    if (val) {
        $("#"+toggleItem).removeClass("inputToggleItem_hidden");
    } else {
        $("#"+toggleItem).addClass("inputToggleItem_hidden");
    }
...
$(".cmsAdminHeader_item_filter").click(function(){
    hasClass = $(".adminCustomFilterList").hasClass("cmsAdminHidden");
    url = "/cms_"+cmsVersion+"/getData/setSession.php";
    if (hasClass) {
        $(".adminCustomFilterList").removeClass("cmsAdminHidden");
        $(this).addClass("cmsAdminHeader_active");
        url += "?adminFilter=1";
    } else {
        $(".adminCustomFilterList").addClass("cmsAdminHidden");
         $(this).removeClass("cmsAdminHeader_active");
        url += "?adminFilter=0";
    }
    url += "&cmsName="+cmsName;
    $.get(url,function(res){
...
// ADMIN SETTINGS                                                             //
$(".adminSetting_listItem").click(function(){
    id = $(this).attr("id");
    id = id.substr(9);
    
    $("#setActiveListItem").val(id);
    
    $(".adminSetting_listItem").removeClass("adminSetting_listItem_selected");
    $(this).addClass("adminSetting_listItem_selected");
    
    $(".adminSettings_list").addClass("adminSettings_hidden");
    
    
    $("#list_"+id).removeClass("adminSettings_hidden");
    
})


$(".adminSetting_inputItem").click(function(){
    id = $(this).attr("id");
    id = id.substr(10);
    
    $("#setActiveInputItem").val(id);
   
    $(".adminSetting_inputItem").removeClass("adminSetting_inputItem_selected");
    $(this).addClass("adminSetting_inputItem_selected");
    
    $(".adminSettings_input").addClass("adminSettings_hidden");
    
    
    $("#input_"+id).removeClass("adminSettings_hidden");
...
$(".cmsToggleItem").live("click",function(){
    ...
    if ($(this).parent().hasClass("cmsToggleNew")) {
        if ($(this).parent().hasClass("cmsToggleSelect_multi")) toggleMode = "multi";
        else toggleMode = "single";
        
        if (toggleMode == "single") {
            if ($(this).hasClass("cmsToggleSelected")) {
                // is selected
            } else {
                $(this).parent().children(".cmsToggleItem").removeClass("cmsToggleSelected");
                $(this).addClass("cmsToggleSelected");
                val = $(this).attr("title");
                $(this).parent().children(".cmsToggleValue").val(val);
            }
        }
...
$(".cmsInput_multiLanguage").click(function(){
    hasClass = $(this).hasClass("cmsInput_multiLanguage_selected");
    if (hasClass) {
        $(this).removeClass("cmsInput_multiLanguage_selected");
    } else {
        $(this).addClass("cmsInput_multiLanguage_selected");
    }
})
...
$(".cmsLgMissing").change(function(){
    state = $(this).attr("checked");
    if (state == "checked") state = 1;
    else state = 0;
    url = "/cms_"+cmsVersion+"/getData/setSession.php";
    url += "?lgMissing="+state;
    url += "&cmsName="+cmsName;
    ...
    if (state) {
        $(".cmsLanguageFlag").removeClass("cmsLanguageFlag_hidden");
    } else {
        $(".cmsLanguageFlag").addClass("cmsLanguageFlag_hidden");
    }
    ...
    // alert ("Change Missing "+state);
}) 
    

$(".cmsSelectDropdown_showSelect").click(function(){
    $(this).parent().children(".cmsSelectDropdown_selectFrame").css("display","inline-block");    

})

$(".cmsSelectDropdown").mouseleave(function(){
     $(this).children(".cmsSelectDropdown_selectFrame").css("display","none");
})


$(".cmsSelectDropdown_selectPos").click(function(){
    old = $(this).parent().parent().children(".cmsSelectDropdown_input").val();
    newText = $(this).text(); //html();

    myClass = $(this).attr("class");
    classList = myClass.split(" ");

    myClass = classList[1];
    myClass = myClass.substr(18);


    // alert(myClass+" old = "+old);

    $(this).parent().children(".cmsSelectDropdown_"+old).removeClass("cmsSelectDropdown_selectPos_selected");


    $(this).parent().parent().children(".cmsSelectDropdown_input").val(myClass);
    $(this).addClass("cmsSelectDropdown_selectPos_selected");

    $(this).parent().parent().children(".cmsTextPos_selectedImage").attr("src","/cms_"+cmsVersion+"/cmsImages/textImagePos_"+myClass+".png");


    $(this).parent().parent().children(".cmsSelectDropdown_showSelect").text(newText);
    $(this).parent().parent().children(".cmsSelectDropdown_selectFrame").css("display","none");

})

$(".cmsSelectIcon_selectPos").click(function(){
    old = $(this).parent().parent().children(".cmsSelectIcon_input").val();
    newText = $(this).text(); //html();
    ...
    myClass = classList[1];
    myClass = myClass.substr(14);


    //alert(myClass+" old = "+old);
//
    $(this).parent().children(".cmsSelectIcon_"+old).removeClass("cmsSelectIcon_selectPos_selected");
//
//
    $(this).parent().parent().children(".cmsSelectIcon_input").val(myClass);
    $(this).addClass("cmsSelectIcon_selectPos_selected");
//
//    $(this).parent().parent().children(".cmsTextPos_selectedImage").attr("src","/cms_"+cmsVersion+"/cmsImages/textImagePos_"+myClass+".png");
//
//
...
$(".cmsTextPos_selectPos").click(function(){
    ...
    $(this).parent().parent().children(".cmsSelectDropdown_showSelect").html(newText);
...
$(".cmsEdit_checkbox").click(function(){
    val = $(this).children(".checkboxVal").val();
    // alert ("value"+val)
    if (val=="1") {
        $(this).removeClass("cmsEdit_checkbox_selected");
        $(this).children(".checkboxOut").text("0");
        $(this).children(".checkboxVal").val("0");
        return 1;
    } else {
        $(this).addClass("cmsEdit_checkbox_selected");
        $(this).children(".checkboxOut").text("1");
        $(this).children(".checkboxVal").val("1");
        return 0;
    }
})

$(".cmsEdit_selectLink_show").click(function(){
    hidden = $(this).parent().children(".cmsEdit_selectLink_frame").hasClass("cmsEdit_selectLink_frameHidden");
    if (hidden) {
        // hideOther
        $(".cmsEdit_selectLink_frame").addClass("cmsEdit_selectLink_frameHidden");
        $(this).parent().children(".cmsEdit_selectLink_frame").removeClass("cmsEdit_selectLink_frameHidden");        
    } else {
        $(this).parent().children(".cmsEdit_selectLink_frame").addClass("cmsEdit_selectLink_frameHidden")
    }    
})

$(".cmsEdit_selectLink_changeType").click(function(){
    type = $(this).attr("class");
    type = $(this).children(".cmsToggleValue").attr("class");
    // alert ("Change Type to "+type);    
}) 
$(".cmsEdit_selectLink_typeChange").click(function(){
    type = $(this).attr("title");
    
    frameClass = $(this).parent().parent();
    
    frameClass.children(".cmsEdit_selectLink_input").addClass("cmsEdit_selectLink_frameHidden");
    frameClass.children(".cmsEdit_selectLink_input"+type).removeClass("cmsEdit_selectLink_frameHidden");
        
    mainClass = $(this).parent().parent().parent();
    
    mainClass.children(".cmsEdit_selectLink_type").val(type);
    selectLink_change(mainClass);
})

$(".cmsEdit_selectLink_target").change(function(){
    mainClass = $(this).parent().parent().parent();
    selectLink_change(mainClass);
})
    
$(".cmsEdit_selectLink_url").change(function(){
    mainClass = $(this).parent().parent().parent();
    selectLink_change(mainClass);
})

$(".cmsEdit_selectLink_button").change(function(){
    mainClass = $(this).parent().parent();
    selectLink_change(mainClass);
})

function selectLink_change(mainClass) {
    divs = mainClass.attr("class");
    
    type = mainClass.children(".cmsEdit_selectLink_type").val();
    frameClass = mainClass.children(".cmsEdit_selectLink_frame");
    // typeFrame = frameClass.children(".cmsEdit_selectLink_input"+type);
    save = "type:"+type+"|";
    // page
    page = frameClass.children(".cmsEdit_selectLink_inputpage").children(".cmsSelectType").val();
    save += "page:"+page+"|";
    
    // url 
    // if (type == "url") {
    url = frameClass.children(".cmsEdit_selectLink_inputurl").children(".cmsEdit_selectLink_url").val();
    if (url) save+= "url:"+url+"|";

    target =  frameClass.children(".cmsEdit_selectLink_inputurl").children(".cmsEdit_selectLink_target").val();
    if (target) save += "target:"+target+"|";
    //}
    // if (type == "page") {
    ...
    // button
    button = frameClass.children(".cmsEdit_selectLink_button").val();
    if (button) save += "button:"+button+"|";
    
    style = frameClass.children(".cmsSelectType").val();
    save += "style:"+style+"|";
    
    mainClass.children(".cmsEdit_selectLink_save").val(save);
...
$(".cmsContent_contentName_mergeButton").click(function(){
    if ($(this).parent().children(".cmsContent_contentName_mergeData").hasClass("cmsEditHidden")) {
        $(this).parent().children(".cmsContent_contentName_mergeData").removeClass("cmsEditHidden");
    } else {
        $(this).parent().children(".cmsContent_contentName_mergeData").addClass("cmsEditHidden");    
    }
})
