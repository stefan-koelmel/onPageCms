    ...
    $(".siteMap_Level1").each(function(){
        ...
        $(this).children(".sortSiteMap").children(".siteMap_content").each(function(){
            ...
            if (id) {
                ...
                sortSub = 0;
                subMainPage = id;
                $("#siteMap_Level_"+subMainPage).children(".sortSiteMap").children(".siteMap_content").each(function(){
                    
                    //out += "class for "+id+" = "+$(this).children(".siteMap_item").attr("class")+"<br>";
                    if ($(this).children(".siteMap_item").hasClass("siteMap_item_dynamicContent")) {
                        // out += "NO SORT FOR dynamicItem <br>";
                    } else {
                        subId = $(this).children(".siteMap_item").attr("id");
                        subId = subId.substr(8);
                        name = $(this).children(".siteMap_item").children(".siteMap_showName").text();
                        sortSub++;
                        if (type != "hidden") out += "subSubzeile "+subId+" name = "+name+" sort = "+sortSub+" ";

                        out += " <input type='"+type+"' name='siteMapList["+subId+"][mainPage]' value='"+subMainPage+"' />";
                        out += " <input type='"+type+"' name='siteMapList["+subId+"][sort]' value='"+sortSub+"' />";
                        if (type != "hidden")  out += " <br />";


                        sortSubSub = 0;
                        subSubMainPage = subId;
                        $("#siteMap_Level_"+subId).children(".sortSiteMap").children(".siteMap_content").each(function(){
                            if ($(this).children(".siteMap_item").hasClass("siteMap_item_dynamicContent")) {
                                // out += "NO SORT FOR dynamicItem <br>";
                            } else {
                                subSubId = $(this).children(".siteMap_item").attr("id");
                                subSubId = subSubId.substr(8);
                                name = $(this).children(".siteMap_item").children(".siteMap_showName").text();
                                sortSubSub++;
                                if (type != "hidden") out += "subSubzeile "+id+" name = "+name+" sort = "+sortSubSub+" ";
                                out += " <input type='"+type+"' name='siteMapList["+subSubId+"][mainPage]' value='"+subSubMainPage+"' />";
                                out += " <input type='"+type+"' name='siteMapList["+subSubId+"][sort]' value='"+sortSubSub+"' />";
                                if (type != "hidden")  out += " <br />";
                            }
                        })
...
$(".siteMap_newButton").live("click",function(){
    ...
    // alert("siteMap Id = "+siteMapId+" id = "+$(this).attr("id"));
...
//        maxSort = 1;
    ...
    if (siteMapId != actSiteMapId) {
        ...
        $.get(callUrl,function(text){
            ...
            // alert ("text"+text);
            if (maxSort) $(".newPage_sort").val(maxSort);
         ...
         // maxSort = target.children(".newPage_sort").val();
         maxSort = target.children(".newPageForm").children(".newPage_sort").val();
