...
class cmsFrame_base {
    ...
    function getFrameStyle_data($frameStyle) {
        $styleData = site_session_get(style);
        if (!is_array($styleData)) {
            site_session_set(style,array());
            $styleData = array();
        }
        if (!is_array($styleData[frame])) {
            site_session_set("style,frame",array());
            $styleData[frame] = array();
        }
...
//        }        
        ...
        site_session_set("style,frame,".$frameStyle,$frameData);
