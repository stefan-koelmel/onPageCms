<?php // charset:UTF-8
class cmsType_contactForm_base extends cmsClass_content_show {
    ...
    function contentType_show() {
        ...
        $sendButton = $this->lg("contentType_contactForm","sendMessage");
        echo ("<input type='submit' class='mainInputButton' name='send' value ='$sendButton' />");
    ...
    function contactForm_showGetName($key) {
        $showName = $this->textList[$key];
        if ($showName) return $showName;
        
        $showName = $this->lg("contentType_contactForm",$key."Name");
        if ($showName) {
            $this->textList[$key] = $showName;
            return $showName;
        }
    ...
    function contactForm_showLine_salut($key, $showName, $showType, $needed, $errorKey,  $frameWidth) {
        ...
        $showData =array();
        $showData["empty"] = "Anrede wählen";
        
        echo ($this->editContent_SelectSettings("salut",$salut,"contactForm[$key]",$showData)."<br />");
   ...
   function contactForm_left($text, $needed, $errorKey, $frameWidth) {
    ...
    function contentType_editContent() {
        ...
        $lgType = "contentType_".$this->contentType;
        
        $res = array();
        $res[contactForm] = array();
        $res[contactForm][showName] = $this->lga("content","contactFormTab");
        $res[contactForm][showTab] = "Simple";
        
        $addData = array();
        $addData["text"] = $this->lga($lgType,"emailSender"); //"eMail an Absender";
        ...
        $addData = array();
        $addData["text"] =  $this->lga($lgType,"emailAdmin"); //"eMail an Schaufenster";
        ...
        $addData = array();
        $addData["text"] =  $this->lga($lgType,"emailAdress"); //"eMailAdresse Schaufenster";
        ...
        $addData = array();
        $addData["text"] = $this->lga($lgType,"addDatabase"); //"Eintragen in Datenbank";
        ...
        $res[inputForm] = array();
        $res[inputForm][showName] = $this->lga("content","contactFormInputTab");
        $res[inputForm][showTab] = "Simple";
        $res[inputForm]  = array();
        // MainData
        $addData = array();
        $addData["text"] = $this->lga($lgType,"nameSalut"); //Anrede";
        ...
        $addData["input"] = $input;
        $addData["mode"] = "Simple";
        $res["inputForm"][] = $addData;

        $addData["text"] = $this->lga($lgType,"nameVName"); //"Vorname";
        ...
        $addData["text"] = $this->lga($lgType,"nameNName"); //"Nachname";
        ...
        $addData["text"] = $this->lga($lgType,"nameCompany"); //"Firma";
        ...
        $res["inputForm"][] = $addData;
         ...
         $addData["text"] = $this->lga($lgType,"nameAdress");"Adresse";
        ...
        $addData["text"] = $this->lga($lgType,"namePhone");"Telefon";
        ...
        $addData["text"] = $this->lga($lgType,"nameEMail");"eMail";
        ...
        $addData["text"] = $this->lga($lgType,"nameMessage"); //"Nachricht";
...
function cmsType_contactForm($contentData,$frameWidth) {
    ...
    // echo ("HIER <br />");
    return $contactFormClass->show($contentData,$frameWidth);
}
