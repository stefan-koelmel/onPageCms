...
class cmsType_header_base extends cmsClass_content_show {
    ...
    function header_getSortList($data=array()) {
        $list = array("logo","name","slogan","user","language","search","wireframeSwitch","special");
        if ($this->mobileEnabled) $list[] = "mobileNav";
        
        $sortList = array();
        foreach ($list as $nr => $key) {
            $sort = $data[$key."_sort"];
            if (!$sort) {
                if ($data[$key]) $sort = $nr + 50;
                else $sort = $nr + 100;
            }
            $sortList[$sort] = $key;            
        }
        ksort($sortList);
//        foreach ($sortList as $sort => $value) {
//            echo ("$sort = $value <br>");
//        }
        return $sortList;
    }
    ...
    function contentType_show() {
        ...
        // show_array($textData);
        $sortList = $this->header_getSortList($data);
        
        
        $showHeaderList = array();
        foreach ($sortList as $nr => $key) {
            $show = $data[$key];
            
            $setData = array();
            $setData[show] = $show;
            $setData[frame]  = $data[$key."Frame"];
            $setData[link] = $data[$key."Link"];
           
            $mobileShow = $data[$key."MobileShow"];
            
            // echo ("$key : mobilShow = '$mobileShow' ");
            $mobileShowResult = $this->content_showContent_mobile($mobileShow,"div");
            
            if (!is_null($mobileShowResult)) {
                $setShow = null;

                if (is_string($mobileShowResult)) {
                    $setData[mobileItemDiv] = $mobileShowResult;
                    $setShow = 1;
                    // echo ("mobilDiv for $key = $mobileShowResult <br />");
                } else {
                    if ($mobileShowResult) $setShow = 1;
                    else $setShow = 0;
                }
                if (!is_null($setShow)) $setData[show] = $setShow;
            }
            
            $showHeaderList[$key] = $setData;
        }
        ...
        foreach ($showHeaderList as $key => $data) {
            $show = $data[show];
            $frame = $data[frame];
            $goLink = $data[link];
            if ($goLink == "noLink") $goLink = 0;
            
            if (!$show) continue;
            
            $str = "";
            
            $itemDivName = "headerItem";
            
            switch ($key) {
                case "logo" :
                    $itemDivName .= " headerLogo";
                    $useWidth = $posData[$frame."_width"];

                    if ($goLink) {
                        $linkPageData = $this->page_getData(intval($goLink));
                ...
                case "wireframeSwitch" :
                    ...
                    if ($wireFrame_enabled) {
                       $itemDivName .= " headerWireFrame";
                       $str .= $this->headerWireframe($contentData,$textData);
                    }
                    break;

                case "special" :
                    $itemDivName .= " headerSpecial";
                    $str .= $this->headerSpecial($contentData,$textData);
                    break;
                
                case "mobileNav" :
                    $itemDivName .= " headerMobilNav";
                    $str .= $this->headerMobilNavigation();
                    break;
            ...
            if ($str) {
                $itemStr = "";
                if ($data[mobileItemDiv]) {
                    $itemDivName .= $data[mobileItemDiv];
                    // $itemStr .= "class='$itemDivName'<br>";                    
                }
                $itemStr .= div_start_str($itemDivName);
                $itemStr .= $str;
                $itemStr .= div_end_str($itemDivName);

                switch ($frame) {
                    case "top" : $topFrame .= $itemStr."\n"; break;
                    case "left" : $leftFrame .= $itemStr."\n"; break;
                    case "middle" : $middleFrame .= $itemStr."\n"; break;
                    case "center" : $middleFrame .= $itemStr."\n"; break;
                    case "right" : $rightFrame .= $itemStr."\n"; break;
                    case "bottom" : $bottomFrame .= $itemStr."\n"; break;
                    default : 
                        echo("dontAdd because $frame <br>");
                }  
            }
        }
        ...
        if ($leftFrame OR $middleFrame OR $rightFrame) {
            ...
            if ($leftFrame) {
                ...
                if ($this->mobileEnabled) {
                    $width = $posData[left_width];
                    if ($width) {
                        $proz = 100.0 * $width / $this->innerWidth;
                        $style .= "width:".$proz."%;";
                    }
                    $abs = $posData[left_abs];
                    if ($abs) {
            ...
            if ($middleFrame) {
                $style = "";
                if ($this->mobileEnabled) {
                    $width = $posData[center_width];
                    if ($width) {
                        $proz = 100.0 * $width / $this->innerWidth;
                        $style .= "width:".$proz."%;";
                    }
                    $abs = $posData[center_abs];
                    if ($abs) {
                        $proz = 100.0 * $abs / $this->innerWidth;
                        $style .= "margin-right:".$proz."%;";
                    }                    
            ...
            if ($rightFrame) {
                $style = "";
                if ($this->mobileEnabled) {
                    $width = $posData[right_width];
                    if ($width) {
                        $proz = 100.0 * $width / $this->innerWidth;
                        $style .= "width:".$proz."%;";
                    }
                    $abs = $posData[right_abs];
                    if ($abs) {
                        $proz = 100.0 * $abs / $this->innerWidth;
                        $style .= "margin-right:".$proz."%;";
                    }                    
    ...
    function contentType_showOld() {
        echo ("<h1>old Header </h1>");
        $contentData = $this->contentData;
        $frameWidth = $this->innerWidth;
        ...
        $mobilShow_logo = $data[logo];
        $mobilShow_name = $data[name];
        $mobilShow_slogan = $data[slogan];
        $mobilShow_user = $data[user];
        $mobilShow_language = $data[language];
        $mobilShow_search = $data[search];
        $mobilShow_wireframeSwitch = $data[wireframeSwitch];
        $mobilShow_special = $data[special];
        $mobilShow_basket = $data[basket];
...
//        }
        ...
        if ($show_logo) {
            ...
            if ($logoLink) {
                $linkPageData = $this->page_getData(intval($logoLink));
                if (is_array($linkPageData)) {
                    $goLink = $linkPageData[name].".php";
                    $goName = $this->text_getFromArray($linkPageData[title]);
        ...
        if ($show_name) {
            ...
            if ($nameLink) {
                $linkPageData = $this->page_getData(intval($nameLink));
                if (is_array($linkPageData)) {
                    $goLink = $linkPageData[name].".php";
                    $goName = $this->text_getFromArray($linkPageData[title]);                   
                }                
        ...
        if ($show_slogan) {
            ...
            if ($sloganLink) {
                $linkPageData = $this->page_getData(intval($sloganLink));
                if (is_array($linkPageData)) {
                    $goLink = $linkPageData[name].".php";
                    $goName = $this->text_getFromArray($linkPageData[title]);                    
                }                
...
//                    $showData[frameWidth] = $data[imageWidth]*100;
    ...
    function headerLanguage($contentData,$textData) {
        $activeLanguage = $this->showLg; // cms_text_getLanguage();
        $languageList = cms_text_getSettings();
        
       
        
        $useMode = "enabled";
        if ($this->session_get(editable)) $useMode = "editable";
        $lgList = array();
        ...
        $wirframeState = $this->wireframeState; 
        if ($wirframeState) {
            ...
            foreach ($lgList as $key => $languageName) {
                if ($key == $activeLanguage) {
                    $active = 1;
                    $className = "cmsLanguageSelect cmsLanguageSelect_$key cmsLanguage_selected ";
                } else {
                    $active = 0;
                    $className = "cmsLanguageSelect cmsLanguageSelect_$key";
                }
    ...
    function headerMobilNavigation () {
        $str = div_start_str("mobileNavigationShow");
        $str .= "NAV";
        $str .= div_end_str("mobileNavigationShow");
        return $str;
    }
    ...
    function contentType_editContent() {
        $old = 0;
        if ($old) {
            return $this->contentType_editContentOld();
        }
        ...
        $goLinkStr  = " ".$this->lga($lgaCode,"goLink",":");
        
        $textStr = $this->lga($lgaCode,"Text",":");
        $textShowData = array();
        $textShowData[out] = "input";
        $textShowData[width] = 200;
        $textShowData[viewMode] = "line";
        
        $mobileShowStr = $this->lga($lgaCode,"showMobile",":");
        $mobileShowData = array("mode"=>"simpleShow","viewMode"=>"select");
        ...
        foreach ($sortList as $sort => $key) {
            ...
            $code = "user";
            $addData = array();
            $addData["text"] = $this->lga($lgaCode,"show".$upKey); //"Benutzer zeigen";
            if ($data[$key]) $checked="checked='checked'";
            else $checked = "";
            $input = "<input type='checkbox' $checked name='editContent[data][$key]' value='1' />";
            $input .= $showTarget.$this->selectPosition($data[$key."Frame"],"editContent[data][".$key."Frame]",array("mode"=>"box"));
            if ($this->mobileEnabled) {
                $input .= $mobileShowStr;
                $input .= $this->editContent_selectSettings("mobileShow",$data[$key."MobileShow"], "editContent[data][".$key."MobileShow]", $mobileShowData); 
            }
            if ($sort) {
                $input .= "<input class='inputLineSortNr' type='hidden_' value='".$data[$key."_sort"]."' name='editContent[data][".$key."_sort]' style='width:30px;' />";
                $addData[sort] = 1;
            }
            $showLink = 0;
            $showText = 0;
            switch ($key) {
                case "logo"   : $showLink = 1; $showText = 0; break;
                case "name"   : $showLink = 1; $showText = 1; break;
                case "slogan" : $showLink = 1; $showText = 1; break;                
            }
            if ($showLink) {
                $input .= $goLinkStr;
                $input .= $this->editContent_SelectSettings("link", $data[$key."Link"],"editContent[data][".$key."Link]",$showLinkData);
            }
            if ($showText) {
                $input .= "<br />".$textStr;
                $input .= $this->editContent_text($key,$this->textData,$textShowData);                
            }
    ...
    function contentType_editContentOld() {
        $editContent = $this->editContent;
        $frameWidth = $this->frameWidth;
        ...
        if ($this->mobileEnabled) {
            $mobileCode = "logo";
            $input .= $mobileShowStr;
            $input .= $this->editContent_selectSettings("mobileShow",$data[$mobileCode."MobileShow"], "editContent[data][".$mobileCode."MobileShow]", $mobileShowData); 
        }
        $input .= $goLink.$this->editContent_SelectSettings("link", $data[logoLink],"editContent[data][logoLink]",$showLinkData);
        if ($sort) {
            $input .= "<input class='inputLineSortNr' type='hidden_' value='".$data[$code."_sort"]."' name='editContent[data][".$code."_sort]' style='width:30px;' />";
            $addData[sort] = 1;
        }
        $addData["input"] = $input;
        $addData["mode"] = "Simple";
...
//        
        ...
        if ($sort) {
            $input .= "<input class='inputLineSortNr' type='hidden_' value='".$data[$code."_sort"]."' name='editContent[data][".$code."_sort]' style='width:30px;' />";$addData[sort] = 1;
        }
        $emptyBasket = $data[emptyBasket];
        ...
        if ($this->mobileEnabled) {
            $code = "mobileNav";
            $addData = array();
            $addData["text"] = $this->lga($lgaCode,"showMobileNav"); //"Sonderanzeige";
            if ($data[$code]) $checked="checked='checked'";
            else $checked = "";
            $input = "<input type='checkbox' $checked name='editContent[data][$code]' value='1' />";
            $input .= $showTarget.$this->selectPosition($data[$code."Frame"],"editContent[data][".$code."Frame]",array("mode"=>"box"));        
            if ($this->mobileEnabled) {
                $input .= $mobileShowStr;
                $input .= $this->editContent_selectSettings("mobileShow",$data[$code."MobileShow"], "editContent[data][".$code."MobileShow]", $mobileShowData); 
            }
            if ($sort) {
               $input .= "<input class='inputLineSortNr' type='hidden_' value='".$data[$code."_sort"]."' name='editContent[data][".$code."_sort]' style='width:30px;' />";$addData[sort] = 1;
            }
