<?php // charset:UTF-8
class cmsType_map_base extends cmsClass_content_show {
   ...
   function contentType_show() {
        ...
        if ($url) {
           ...
           //  echo ("Map $zoomStart<br />");
            ...
            $urlMap = "https://www.google.de/maps?f=q&source=s_q&hl=en&geocode=&q=Fasanenstra%C3%9Fe+17,+Karlsruhe&aq=0&oq=Fasanenstra%C3%9Fe+17,+Karlsruhe&sll=51.175806,10.454119&sspn=10.267703,19.753418&t=h&ie=UTF8&hq=&hnear=Fasanenstra%C3%9Fe+17,+Innenstadt-Ost+76131+Karlsruhe,+Baden-W%C3%BCrttemberg&z=14";
            ...
            switch ($this->showLg) {
                case "dt" : $urlMap .= "&hl=de"; break;
                case "en" : $urlMap .= "&hl=en"; break;
                case "fr" : $urlMap .= "&hl=fr"; break;
                default :
                    echo ("unkown Language $this->showLg use en <br />");
                    $urlMap .= "&hl=en";
            }
    ...
    function contentType_editContent() {
        ...
        $res = array();
        $res[map] = array();
        $res[map][showName] = $this->lga("content","mapTab");
        $res[map][showTab] = "Simple";
...
//        $res[map][] = $addData;
        ...
        if (!$zoom ) $zoom = 16;
        $addData = array();
        $addData["text"] = $this->lga("contentType_map","googleUrl");//"GoogleUrl";
        ...
        $addData = array();
        $addData["text"] = $this->lga("contentType_map","viewMode");// "Ansicht";
        $viewList = array();
        $viewList["m"] = $this->lga("contentType_map","viewMap");
        $viewList["h"] = $this->lga("contentType_map","viewSatelite");
        $viewList["p"] = $this->lga("contentType_map","viewTer");
        $viewList["e"] = $this->lga("contentType_map","viewEarth");
        ...
        if (!$zoom ) $zoom = 16;
        $addData = array();
        $addData["text"] =  $this->lga("contentType_map","zoom");// "Zoom";
        ...
        // ShowInfoBox
        $addData = array();
        $addData["text"] = $this->lga("contentType_map","showInfo");//"InfoBox zeigen";
...
function cmsType_map($contentData,$frameWidth) {
    ...
    return $mapClass->show($contentData,$frameWidth);
}
