...
class cmsType_search_base extends cmsClass_content_show {
    function getName (){
        return "Suche";        
    }
    ...
    function contentType_show() {
        ...
        $search = $useClass->session_get(search);
        if (is_array($search)) {
...
//                }
    ...
    function show_input($searchString,$contentData,$frameWidth) {
        ...
        $data = $useClass->contentData[data];
        if (!is_array($data)) $data = array();
        $contentId = $useClass->contentId;
        $str .= div_start_str("searchInputFrame",array("id"=>"searchInput_".$contentData[id]));
        //echo ("<form method='post' >"); 
        $lgStr = $useClass->lg("search","searchText",":");
        if ($lgStr) {
        ...
        $str .= "<div class='searchCancelButton mainJavaButton mainSmallButton mainSecond' >".$useClass->lg("search","deleteButton")."</div>";
        $str .= "<div class='searchSearchButton mainJavaButton'>".$useClass->lg("search","searchButton")."</div>";
        ...
        foreach ($totalSearchResult as $foundIn => $foundData) {
            ...
            if (is_array($searchResult) AND count($searchResult)) {
                ...
                if ($anz > 0) {
                    ...
                    $str .= "$anz Treffer im Bereich <k>$searchName</k>";
...
//            echo ("<a href='$goPage' >");
        ...
        foreach ($hit as $key => $value ) {
            ...
            $helpArray = str2Array($value);
            if (is_array($helpArray)) {
                $value = $this->lgGet($helpArray);
                // if ($helpArray[$this->session_get(lg]]) $value = $helpArray[$this->session_get(lg]];
//                $str = "";
//                foreach ($helpArray as $key => $value) $str.= "$key |";
//                $value = "ar ($str)";
            }
            ...
            switch ($key) {
                ...
                case "text" :
                    if ($show) {
                        $res .= "<div class='searchHit_$key' >";
                        if (is_array($value)) $str = "TEXT is array ";
                        else $str = "TEXT: $value"; //  $this->hit_String($value,$length);                        
                        $res .= "$str<br />";
                        $res .= "</div>";
                    }
                    break;
    ...
    function search_result($contentData,$searchString) {
        ...
        if ($cache) {
            $search = $this->session_get(search);
            if (is_array($search)) {
                $useSearch = 1;
                ...
                if ($useSearch) {
                    ...
                    $this->session_set("search,lastSearch",$searchString);
        ...
        foreach ($searchList as $searchKey => $searchValue) {
            ...
            if ($search) {
                ...
                // echo ("notSearch in $searchKey <br>");
            }
        }   
        $search = array();
        ...
        $search[result] = $res;
        $this->session_set(search,$search);
        return $res;
    }
    ...
    function search_result_page($searchString,$searchValue,$searchText,$contentData,$pageData) {
        ...
        for ($i=0;$i<count($pageList);$i++) {
            ...
            //if (is_array($pageText)) echo ("is ARRAY ");
            // echo ("pageText = '$pageText' <br>");
...
//                    
    ...
    function search_result_text($searchString,$searchValue,$searchText,$contentData,$pageData) {
        if (!function_exists("cmsText_search")) {
            $dataFile = $_SERVER['DOCUMENT_ROOT']."/cms_".$GLOBALS[cmsVersion]."/data/cms_text.php";
            $error = 1;
            if (file_exists($dataFile)) {
                // echo ("Data File exist<br>");               
                include ($dataFile);                
...
//                    
    ...
    function contentType_editContent() {
        ...
        $res[search][showName] = $useClass->lga("content","searchTab");
        $res[search][showTab] = "Simple";

        $lgaCode = "contentType_search";
        $addData = array();
        $addData["text"] = $useClass->lga($lgaCode,"inWindow"); // "Treffer im Fenster";
        ...
        // MainData
        $addData = array();
        $addData["text"] = $useClass->lga($lgaCode,"hitsPerArea"); // "Treffer pro Bereich";
        $addData["input"] = "<input type='text' name='editContent[data][hitCount]' value='$data[hitCount]' style='width:50px;' />";
        $addData["mode"] = "Simple";
        $res[search][] = $addData;

        $addData = array();
        $addData["text"] = $useClass->lga($lgaCode,"showHeadline"); //"Ausgabe Überschrift";
        ...
        $addData = array();
        $addData["text"] = $useClass->lga($lgaCode,"show2HeadLine"); //"Ausgabe 2. Überschrift";
        ...
        $maxCharText = $useClass->lga($lgaCode,"maxChar",": "); //
        $addData = array();
        $addData["text"] = $useClass->lga($lgaCode,"showText"); //"Ausgabe Text";
        if ($data[show_text]) $checked = "checked='checked'";
        else $checked = "";
        $input = "<input type='checkbox' $checked name='editContent[data][show_text]' value='1' />";
        $input .= $maxCharText."<input type='text' name='editContent[data][show_text_length]' value='".$data["show_text_length"]."' style='width:50px;' />";
        $addData["input"] =  $input;
        $addData["mode"] = "Simple";
        $res[search][] = $addData;

        $addData = array();
        $addData["text"] = $useClass->lga($lgaCode,"showLongTex"); //"Ausgabe Langer Text";
        ...
        $input .= $maxCharText."<input type='text' name='editContent[data][show_longText_length]' value='".$data["show_longText_length"]."' style='width:50px;' />";
        $addData["input"] =  $input;
        $addData["mode"] = "Simple";
        $res[search][] = $addData;
        ...
        $res[searchIn] = array();
        $res[searchIn][showName] = $useClass->lga("content","searchInTab");
        $res[searchIn][showTab] = "More";
        $searchList = $this->searchList();
        $searchContentText =  $useClass->lga($lgaCode,"searchContext",": ");
        $searchTargetText =  $useClass->lga($lgaCode,"targetPage",": ");
        $searchInText =  $useClass->lga($lgaCode,"searchIn");
       
        foreach($searchList as $key => $value) {
            $name = $value[name];
            $searchInType =  $useClass->lga($lgaCode,"searchType_".$key);

            $addData = array();

            $addData[text] = $searchInText." <b>$searchInType</b>"; //Suchen in <b>$name</b>";
            ...
            $input .= $searchContentText."<input type='checkbox' $checked value='1' name='editContent[data][search_".$key."_text]' />";
            
            if ($value[pageSelect]) {
                $pageId = $data["search_".$key."_page"];
                $input .= $searchTargetText; //.cms_page_SelectMainPage($pageId,"editContent[data][search_".$key."_page]");
                $input .= $useClass->editContent_SelectSettings("link", $pageId,"editContent[data][search_".$key."_page]",$showLinkData);
            }
...
function cmsType_search($contentData,$frameWidth) {
    ...
    $res = $searchClass->contentType_show($contentData,$frameWidth);
    return $res;
}
...
function cmsType_search_editContent($editContent) {
    $searchClass = cmsType_search_class();
    $res = $searchClass->contentType_editContent($editContent, $frameWidth);
    return $res;
}
