...
class cmsType_user_base extends cmsClass_content_show {
    function getName (){
        return "Benutzer";        
    }
   ...
   function contentType_show() {
    ...
    function user_showList() {
        
        $userList = cmsUser_getList($filter,"");
        $this->data_showList("user",$userList);
...
//        echo ("Confirm:'$confirm'<br>");
    ...
    function show_emailResetLink($contentData,$frameWidth) {
        ...
        if ($res) {
            ...
            if ($this->userLevel>9) show_array($saveData);
    ...
    function show_userLogin($contentData,$frameWidth) {
        $showLevel = $this->session_get(showLevel);
                ...
                if ($newPass != $newPass2) {
                    ...
                    if (strlen($newPass)<6) {
                        ...
                        if ($newPass == $oldPass) {
                            ...
                            $userData = cmsUser_get(array("id"=>$this->session_get(userId)));
                            if (is_array($userData)) {
...
//        $res[showTab] = "Simple";
        ...
        $res[showName] = $this->lga("content",$this->tableName."notLoggedTab");
        $res[showTab] = "Simple";
    ...
    function user_edit_userShow($editContent,$frameWidth) {
        ...
        $res[showName] = $this->lga("content",$this->tableName."ViewTab");
        $res[showTab] = "Simple";
...
function cmsType_user($contentData,$frameWidth) {
    ...
    return $userClass->show($contentData,$frameWidth);
}
