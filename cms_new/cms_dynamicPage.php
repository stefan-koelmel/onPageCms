...
//            }
            ...
            switch ($dynamic_1_type) {
                case "category" :
                    ...
                    for ($i=0;$i<count($catList);$i++) {
                        ...
                        if (is_array($catName)) $catName = cms_text_getLg ($catName);
                ...
                case "project" :
                    ...
                    for ($i=0;$i<count($projectList);$i++) {
                        ...
                        if (is_array($projName)) $projName = cms_text_getLg($projName);
                        if ($urlAdd) {
            ...
            case "product" :
                    ...
                    for ($i=0;$i<count($productList);$i++) {
                        ...
                        if (is_array($prodName)) $prodName = cms_text_getLg ($prodName);
                        if ($urlAdd) {
                ...
                case "company" :
                    ...
                    for ($i=0;$i<count($companyList);$i++) {
                        ...
                        if (is_array($compName)) $compName = cms_text_getLg ($compName);
                        if ($urlAdd) {
                ...
                case "article" :
                    ...
                    for ($i=0;$i<count($articleList);$i++) {
                        ...
                        if (is_array($articleName)) $articleName = cms_text_getLg ($articleName);
                        if ($urlAdd) {
                ...
                case "dates" :
                    ...
                    for ($i=0;$i<count($dateList);$i++) {
                        ...
                        if (is_array($dateName)) $dateName = cms_text_getLg ($dateName);
                        if ($urlAdd) {
...
function cms_dynamicPage_getList_project($dynamicData,$selectId,$showData) {
    ...
    for ($i=0;$i<count($projectList);$i++) {
        ...
        $projId = $projData[id];
        $projName = $projData[name];
//        if (is_array($projName)) echo "is ARRAy - ";
//        echo ("FOUND project $projName $i <br>");
...
function cms_dynamicPage_getSortList($pageData) {
    ...
    if (is_array($list) AND count ($list)) {
        ...
        if (is_array($pageTitle)) $pageTitle = cms_text_getLg ($pageTitle);
        foreach ($list as $projId => $subData) { // ($i = 0;$i<count($list);$i++) {
            // echo ("Add Dynamic $pageData[name] $dataSource to $page_name $projId $subData <br> ");
            $name = $subData[name];
            if (is_array($name)) $showName = cms_text_getLg ($name);
            else $showName = $name;
            
            
            $pageCode = $pageData[name]."_".$dataSource."=".$projId;
            // echo ("addPageCode $pageCode <br>");
            // $subPageName = "$pageData[title]_$name";
            $subPageName = $pageTitle."_".$showName;
            ...
            // echo ("ADD DYNAMIC $subPageName Page $name <br>  ");
            ...
            if ($dataSource2) {
                ...
                if (is_array($subList) AND count ($subList)) {
                    foreach ($subList as $projId => $subSubData) { // ($i = 0;$i<count($list);$i++) {
                        ...
                        $subPageCode = $pageCode."|".$dataSource2."=".$projId;
                        ...
                        $subNavi[$subPageCode] = $subSubPageData;
...
function cms_dynamicPage_Source($pageId,$dataName) {
    ...
    if (!$pageId) $selected = "selected='selected'"; else $selected ="";
    $str .= "<option value='0' $selected >keine Quelle</option>";
