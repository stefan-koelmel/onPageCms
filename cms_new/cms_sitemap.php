...
class sitemap_base extends cmsType_contentTypes_base {
    function init() {
        ...
        $this->edit = site_session_get(edit);
        $this->editAble = site_session_get(showLevel)>6;
        $this->myLevel = site_session_get(showLevel);
        if (!$this->myLevel) $this->myLevel = 0;
        $this->myId = site_session_get(userId);
        $this->sortAble = 1;
                    ...
                    if (is_array($subPage[subNavi]) AND $showSubNavi) {
                        ...
                        foreach ($subPage[subNavi] as $subSubIdCode => $subSubPage) {
                            ...
                            if (is_array($subSubPage[subNavi]) AND $showSubNavi) {
                                ...
                                $level = 3;
                                $subSubSubDivName = "siteMap_Level4 siteMap_Level";
                                if ($level >= $this->closeLevel) $subSubSubDivName .=" siteMap_Level_hidden";
                                div_start($subSubSubDivName,array("id"=>"siteMap_Level_".$subSubPage[id]));
                                echo ("SEITE MIT LEVEL 4<br>");
                                
                                div_start("sortSiteMap sortSiteMap_4");
                                $subSubSort = 0;
                                foreach ($subSubPage[subNavi] as $subSubSubIdCode => $subSubSubPage) {
                                    $sort = $subSubSubPage[sort];
//                                    if ($sort>$subSubSubSort) $subSubSubSort = $sort; 
                                    $this->showItem_start($subSubSubPage,4);
//                                    if (is_array($subSubSubPage[subNavi]) AND $showSubNavi) {
//
//
//
//
...
//
                                ...
                                div_end("sortSiteMap sortSiteMap_4");
                        
                                $mainPage = $subPage[id];
                                if ($this->editAble) $this->show_NewPage($mainPage,$subSubSort);
                        
                                div_end($subSubSubDivName);
...
//        echo (span_text_str($showName,100));
        ...
        echo ("neue Seite $mainPage max=$maxSort ");
                ...
                if (is_array($saveList)) {
                    foreach ($saveList as $pageId => $sortData) {
                        ...
                        if ($pageData[sort] != $sort) {
                            echo ("Change Sort for $pageId / $pageData[name] from $pageData[sort] to $sort <br />");
                            $sortData[id] = $pageId;
                            ...
                            // echo ("NO CHANGE in SORT $pageId from $pageData[sort] == $sort<br />");
                        }
...
function cms_siteMap_deletePage($pageId) {
    ...
    if (is_array($showName)) {
        $lgStr = $showName[cms_text_getLanguage()];
        if (is_string($lgStr)) $showName = $lgStr;
        else {
            $str .= cms_text_adminLg()." . ".$lgStr;
        
            foreach ($showName as $key => $value) $str .= "$key = $value <br>";
        }
    }
...
function cms_siteMap_addPageForm($pageId) {
    ...
    echo ("Neue Seite einfügen unterhalb von <b>$title</b><br>");
    ...
    echo ("<form method='post' class='newPageForm' >\n");

    echo ("addTo=<input type='hidden_' name='addPage[addPageTo]' value='$pageId' />");
    echo ("sort=<input type='hidden_' class='newPage_sort' name='addPage[sort]' value='' />");
