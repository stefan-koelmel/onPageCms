...
function cmsArticles_getList($filter,$sort,$out="normal") {
    if ($sort) {
                    ...
                    switch ($key) {
                        ...
                        case "date" :
                            ...
                            switch ($start) {
                                ...
                                case "<=" : // PAST
                                    $value = substr($value,2);
                                    $filterQuery .= "`toDate` <= '$value'";
                                    break;
                                default :
                                    $filterQuery .= "(`fromDate` <= '$value' AND `toDate` >= '$value')";
                                    
                            }
                            // echo ("QUERY = $filterQuery <br>");
                            // $filterQuery .= "(`fromDate` <= '$value' AND `toDate` >= '$value')";
                            break;
                        default :
                            $add = "";
    ...
    $query = "SELECT * FROM `".$GLOBALS[cmsName]."_cms_articles` ".$filterQuery." ".$sortQuery;
    if ($out == "out") echo("<h4>Query $query </h4>");
    if ($out == "query") {
        $query2 =  "SELECT `id` FROM `".$GLOBALS[cmsName]."_cms_articles` ".$filterQuery." ".$sortQuery;
    ...
    while ($articles = mysql_fetch_assoc($result)) {
        $articles = php_clearQuery($articles);
//         if ($articles[data]) {
//            $data = str2Array($articles[data]);
//
//            if (is_array($data)) {
//                $articles[data] = array();
//                foreach ($data as $key => $value) {
//                    $articles[data][$key] = php_unclearStr($value);
//                }
//            }
//        }
//        foreach ($articles as $key => $value) {
//            switch ($key) {
//                case "name"    : $articles[$key] = php_unclearStr($value); break;
//                case "subName" : $articles[$key] = php_unclearStr($value,1); break;
//                case "info"    : $articles[$key] = php_unclearStr($value); break;
//                case "data"    :
//                    $data = str2Array($articles[data]);
//                    if (is_array($data)) {
//                        $articles[data] = array();
//                        foreach ($data as $key2 => $value2) {
//                            $articles[data][$key2] = php_unclearStr($value2);
//                        }
//                    }
//                    break;
//            }
...
function cmsArticles_search($searchString,$searchText,$filter) {
    if (!is_array($filter)) $filter = array();
    if ($searchText) $filter["searchText"] = $searchString;
    else $filter["search"] = $searchString;
    $out = "";
    $sort ="";
    $res = cmsArticles_getList($filter,$sort,$out);
    return $res;
}


function cmsArticles_getById($articlesId) {
    $articlesData = cmsArticles_get(array("id"=>$articlesId));
    return $articlesData;   
}


function cmsArticles_get($filter,$clearData=1) {
    $filterQuery = "";
    if (is_array($filter)) {
        foreach ($filter as $key => $value) {
   ...
   //  echo("$query -> $result $anz <br>");
    ...
    $articlesData =  mysql_fetch_assoc($result);
    foreach ($articlesData as $key => $value) {
        if ($clearData) $articlesData = php_clearQuery($articlesData);
//        switch ($key) {
//            case "data" :
//                $data = str2Array($articlesData[data]);
//                if (is_array($data)) {
//                    $articlesData[data] = array();
//                    foreach ($data as $key2 => $value2) {
//                        $articlesData[data][$key2] = php_unclearStr($value2);
//                    }
//                }
//                break;
//            default :
...
function cmsArticles_existID($id,$clearData=1) {
    $res = cmsArticles_get(array("id"=>$id),$clearData);
    return $res;
}


function cmsArticles_save($data,$clearData=1) {
    ...
    if ($id) {
        // echo ("id exist $id <br>");
        $existData = cmsArticles_existID($id,$clearData);
        if (is_array($existData)) {
            // if (is_array($existData[data])) $existData[data] = array2Str($existData[data]);
            return cmsArticles_update($data,$existData,$clearData);
        }
    }
...
//    $existData = cmsArticles_get(array("name"=>$name,"category"=>$data[category]));
//    if (is_array($existData)) {
//        // echo ("Found with Name <br>");
//        $data[id] = $existData[id];
//        return cmsArticles_update($data,$existData);
//    }
    ...
    $query = "INSERT INTO `".$GLOBALS[cmsName]."_cms_articles` SET ".$query;
    $result = mysql_query($query);
    if (!$result) {
        echo "Error in cmsArticles_save $query <br>";
        return 0;
    }
    return 1;
}

function cmsArticles_update($data,$existData=array(),$clearData=1) {
    $query = "";
    $id = $data[id];
...
//    } else echo ("Sort is not set sort = '$data[sort]' highlight =  '$data[highlight]' <br>");
    ...
    $query = "UPDATE `".$GLOBALS[cmsName]."_cms_articles` SET ".$query." WHERE `id` = $id ";
    // echo ("Query $query<br>");
    $result = mysql_query($query);
    if (!$result) {
...
function cmsArticles_selectArticles($code,$dataName,$showData,$filter,$sort) {
    ...
    for($i=0;$i<count($articlesList);$i++) {
        $articleId = $articlesList[$i][id];
        $outValue = "name";
        if ($showData[out]) $outValue = $showData[out];
        $articleName = $articlesList[$i][$outValue];
