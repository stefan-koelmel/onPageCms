...
function cmsFaq_checkLanguage($editLanguages) {
    $query = "SELECT * FROM `".$GLOBALS[cmsName]."_cms_faq`  ";
    $result = mysql_query($query);
    ...
    foreach ($editLanguages as $lgCode => $value ) {
        $query = "";
        if (is_null($lgData["head_$lgCode"])) {
            $query .= "`head_$lgCode` TEXT NOT NULL";
        }
        // ALTER TABLE `game_cms_faq` ADD `headline_fr` TEXT NOT NULL , ADD `text_fr` TEXT NOT NULL 
        if (is_null($lgData["text_$lgCode"])) {
            if ($query) $query .= ", ";
            $query .= " ADD `text_$lgCode` TEXT NOT NULL";
        }
        
        if ($query) {
            // echo ("<h1> NOT FOUND $lgCode </h1>");
            $query = "ALTER TABLE `".$GLOBALS[cmsName]."_cms_faq` ADD ".$query;
            // echo ("$query<br>");
        
            $result = mysql_query($query);
            if ($res) $res.="<br />";
            if ($result) {
                $res .= "Sprache $lgCode in FAQ angelegt ";
            } else {
                $res.= "Fehler beim Sprache '$lgCode' anlegen !";
            }
        }        
    }
    return $res;
