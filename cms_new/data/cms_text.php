...
function cms_text_getLanguage($setLg=null) {
    if (is_string($setLg)) {
        site_session_set(lg,$setLg);
        
        return $setLg;        
    }
    $lg = site_session_get(lg); //$_SESSION[lg];
    if ($lg) return $lg;
    ...
    site_session_set(lg,$setLanguage);
...
function cms_text_adminLg($setLg=null) {
    if (is_string($setLg)) {
        ...
        site_session_set(adminLanguage, $setLg);
        // $_SESSION[adminLanguage] = $setLg;
        return $setLg;
    }
    
    
    $adminLg = site_session_get(adminLanguage); //$_SESSION[adminLanguage];
    ...
    site_session_set(adminLanguage, $setAdminLg);
...
function cms_text_getLg($str,$showMissing=1,$useDefault=1) {
    global $pageClass;
    if (is_object($pageClass))  {
        $res = $pageClass->lgStr($str,$showMissing,$useDefault);
    ...
    if ($useDefault) {
        foreach($lgList as $lgCode => $lgStr) {
            if ($lgStr) {
                $str = "";
                if (site_session_get(showLevel)>3) $str .= $lgCode.":";
                $str .= $lgStr;
                return $str;
            }
        }
...
function cms_text_getSettings() {
    ...
    $activeLanguage = site_session_get(lg); // $_SESSION[lg];;
    $defaultLanguage = "";
...
function cms_text_lg_edit() {
    $lgEdit = site_session_get(lgEdit); // $_SESSION[lgEdit];
    if ($lgEdit) return $lgEdit;    
    $lgEdit = site_session_get(lg); //$_SESSION[lg]; //cms_text_getLanguage();
    site_session_set(lgEdit,$lgEdit); // $_SESSION[lgEdit] = $lgEdit;
    return $lgEdit;
}

function cms_text_lg_show() {
    $lgShow = site_session_get(lgShow); //$_SESSION[lgShow];
    if ($lgShow) return $lgShow;
    
    $lgShow = "";
    site_session_set(lgShow,$lgShow); //$_SESSION[lgShow] = $lgShow;
    return $lgShow;
}
...
function cms_text_getForContent($contentName,$allLg=0) {
    ...
    if (!$lg) {
        $lg="dt";
        $pageInfo[lg] = $lg;
        site_session_set(lg,$lg); //$_SESSION[lg] = $lg;
