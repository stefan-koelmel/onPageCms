...
function cms_user_login($loginData) {
    ...
    if (!$result) {
        ...
        if ($anz == 1) {
            $userData = mysql_fetch_assoc($result);
            return cmsUser_doLogin($userData);
            //  return (cms_user_doLogin($result));
        }
            //
        if ($anz == 0) {
            // Try eMail
...
function cms_user_doLogin($result) {
    echo ("<h1>SHOULD NOT USE -> use cmsUser_doLogin </h1>");
    $userData = mysql_fetch_assoc($result);
    return cmsUser_doLogin($userData);
    ...
    if ($sessionType == "cmsName") {
        ...
        $_SESSION[$cmsName."_session"][userLevel] = $userData[userLevel];
        $_SESSION[$cmsName."_session"][showLevel] = $userData[userLevel];
        $_SESSION[$cmsName."_session"][userId]    = $userData[id];        
    } 
...
function cmsUser_doLogin($userData) {
    ...
    $sessionType = $_SESSION[sessionType];
    if ($sessionType == "cmsName") {
        site_session_set(userLevel,$userData[userLevel]);
        site_session_set(showLevel,$userData[userLevel]);
        site_session_set(userId,$userData[id]);
        site_session_set(lastPages,array());
        site_session_set(pageGroupList,null);
        site_session_set(pageList,null);
...
function cms_user_logout($userId = 0) {
    ...
    $sessionType = $_SESSION[sessionType];
    if ($sessionType == "cmsName") {
        site_session_set(userLevel,0);
        site_session_set(showLevel,0);
        site_session_set(userId,0);
        site_session_set(lastPages,array());      
...
function cmsUser_selectUserLevel($code,$dataName,$showData,$showFilter,$showSort) {
    ...
    if (!$maxLevel) $maxLevel = site_session_get(userLevel); // $_SESSION[userLevel];
