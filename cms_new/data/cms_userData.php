...
class cms_userData_base  {
    ...
    function bookmarks_toggle($userId,$pageName) {
        // echo ("bookmarks_toggle($userId,$pageName)<br>");
        ...
        if (is_array($res)) {
            $bookmarkId = $res[id];
            // echo ("exist $bookmarkId");
            $this->bookmarks_deleteBookmark($bookmarkId);
           return 0;
            
        } else {
            // echo ("Not Exist");
            $url = "";
            $res = $this->bookmarks_setBookmark($userId,"",$pageName,"");
            // echo ("add $res <br>"); 
            return 1;
...
function cmsUserData_bookmarks_toggle($userId,$pageName) {
    $class = cmsUserData_class();
    $res = $class->bookmarks_toggle($userId,$pageName);
    return $res;
}
