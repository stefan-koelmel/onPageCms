...
function cmsType_owm_addJavascript() {    
    if (file_exists("/cms/contentTypes")) $path = "/cms/cms_contentTypes";
...
function cms_contentType_getSortetList($sortet=0) {
    switch ($sortet) {
        ...
        case "all" :
            ...
            if (is_array($_SESSION[disabledTypes])) {
                foreach ($_SESSION[disabledTypes] as $type => $value) {
...
function cms_contentType_checkType($type) {
    ...
    if (!is_array($_SESSION[cmsSettings])) return "no cmsSettings";
    
    if (!is_array($_SESSION[cmsSettings][useType])) {
        $change = 0;
            ...
            switch ($key) {
                case "data" : 
                    if (is_string($value)) {
                        ...
                        if (is_array($newVal)) {
                            $_SESSION[cmsSettings][$key] = $newVal;
                            $change++;
                        }
                    }
                    break;
...
//            $GLOBALS[cmsSettings] = $
        ...
        if (!is_array($_SESSION[cmsSettings][useType])) {
            echo ("SES = $_SESSION[cmsSettings][useType] GLOB = $GLOBALS[cmsSettings][useType] <br />");
    ...
    // foreach ($_SESSION[cmsSettings][useType] as $key => $value) echo ("ShowType $key => $value <br />");
    $use = intval($_SESSION[cmsSettings][useType][$type]);
