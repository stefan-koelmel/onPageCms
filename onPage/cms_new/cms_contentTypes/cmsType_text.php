...
//        }
    ...
    function text_showHeadLine($headlineData,$id) {
       ...
       if ($useClass->wireFrameState AND $useClass->wireFrameEnabled) {
            // $wireFrameOn AND $wireframeState) {
            $wireOn = $useClass->contentData[wireframe][headLine];
            $wireText = $useClass->contentData[wireframe][headLineText];
            if ($wireOn) {
                $text = $useClass->text_wireText($text,$wireText,array("type"=>"headline"));
            }
        }    
        // echo ("<h1>HAEDLINE $text $wireOn $wireText </h1>");
     
       
        
        
        
        $lgCode = "lg_".$this->showLg;
        $headline = $headlineData[$lgCode];
        if (!$headline AND $headlineData[text]) {
        ...
        if ($headline) {
            ...
            if ($headlineCss) echo ("<$headlineCss>$text</$headlineCss>");
            else echo ("<h4>$text</h4> ");
    ...
    function text_showText($textData,$id) {
        ...
        if ($useClass->wireFrameState AND $useClass->wireFrameEnabled) {
            // $wireFrameOn AND $wireframeState) {
            $wireOn = $useClass->contentData[wireframe][text];
            $wireText = $useClass->contentData[wireframe][textText];
            if ($wireOn) {
                $text = $useClass->text_wireText($textData,$wireText,array("type"=>"text"));
            }
        }    
