                            ...
                            switch ($key) {
                                ...
                                case "searchText" :
                                    if ($value) {
                                        ...
                                        $filterQuery .= " OR `subName` LIKE '%$value%'";
                                        $filterQuery .= " OR `info` LIKE '%$value%'";
                                        $filterQuery .= " OR `street` LIKE '%$value%'";
                                        $filterQuery .= " OR `city` LIKE '%$value%' )";
...
function cmsLocation_search($searchString,$searchText,$filter=array()) {
    if (!is_array($filter)) $filter = array();
    if ($searchText) $filter["searchText"] = $searchString;
    else $filter["search"] = $searchString;
    $out = "";
    $sort ="";
    $res = cmsLocation_getList($filter,$sort,$out);
    return $res;
}
