...
class cmsAdmin_order_base extends cmsAdmin_editClass_base {
    ...
    function show($frameWidth) {
        ...
        switch ($view) {
            case "new" :
             ...
             case "edit" :
                $orderId = $_GET[id];
                $this->show_edit($orderId,$frameWidth);
                break;
    ...
    function show_list($frameWidth) {
        ...
        $orderList = cmsEmail_getList($filter,$sort);

        $this->checkList($orderList,$_data[filter_specialView]);

      

        $showList = $this->order_showList();
        ...
        $this->showList_List($orderList,$showList,$showData,$frameWidth);

        echo ("&nbsp;<br />");
        echo ("<a href='".$GLOBALS[pageData][name].".php?view=new' class='cmsLinkButton' >neue neuen Bestellung anlegen</a>");
    }


    function order_showList() {
        $showList = array();
    ...
    function show_edit($orderId,$frameWidth) {
        if ($_POST[saveData][id]>0) $orderId = $_POST[saveData][id];
        if ($orderId > 0) {
            $mode = "edit";
            $headLine = "eMail bearbeiten";
            $saveData = cmsEmail_getById($orderId);
            if ($_POST[saveData]) {
        ...
        if (is_array($_POST)) {
            ...
            foreach ($specialPostList as $key => $value ) {
                switch ($key) {
                    ...
                    case "orderId" : $saveData[order] = $value; break;
                    case "orderName" : $orderName = $value; break;
       ...
       // show_array($saveData);
        ...
        if ($_POST[editSave]) {
            ...
            if (count($error) == 0 AND is_array($saveData)) {
                ...
                if ($mode == "new") {
                    $query = "INSERT INTO `".$GLOBALS[cmsName]."_cms_order` SET $query  ";
                }
                if ($mode == "edit") {
                    $query = "UPDATE `".$GLOBALS[cmsName]."_cms_order` SET $query WHERE `id` = $saveDataId ";
                }
                echo ("$query<br />");
...
function cms_admin_order($frameWidth,$ownAdminPath=""){
    $ownPhpFile = $ownAdminPath."/cms_admin_order_own.php";
    if (file_exists($ownPhpFile)) {
        require_once($ownPhpFile);
        $class = new cmsAdmin_order();

    } else {
        $class = new cmsAdmin_order_base();
