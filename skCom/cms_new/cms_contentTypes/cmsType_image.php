...
class cmsType_image_base extends cmsClass_content_show { // extends cmsType_contentTypes_base {
    ...
    function contentType_show() {
        ...
        $res = $this->image_show($contentData,$frameWidth);
        return $res;
...
//    function image_editContent($editContent,$useClass->frameWidth) {
        ...
        $res[image][showName] = $useClass->lga("content","imageText");
        $res[image][showTab] = "Simple";
        
        
        $addData = array();
        $addData[text] = $useClass->lga("contentType_image","displayMode");
...
//                    }
    ...
    function editContent_singleImageSettings() {
        ...
        // MainData
        $addData = array();
        $addData["text"] = $useClass->lga("contentType_image","selectImage");
        $input = "";
    ...
    function editContent_imageListSettings() {
        ...
        $imgRow = $data[imgRow];
        if (!$imgRow) $imgRow = 3;
        $addData["text"] = $useClass->lga("contentType_image","imageRowCount"); //"Anzahl Bilder in Reihe";
        ...
        $imgRowAbs = $data[imgRowAbs];
        if (!$imgRowAbs) $imgRowAbs = 10;
        $addData["text"] = $useClass->lga("contentType_image","imageRowDist"); //"Abstand Bilder in Reihe";
        ...
        $imgColAbs = $data[imgColAbs];
        if (!$imgColAbs) $imgColAbs = 10;
        $addData["text"] = $useClass->lga("contentType_image","imageColumnDist"); //"Abstand Zeilen";
        ...
        $res[imageList] = array();
        $res[imageList][showName] = $useClass->lga("content","imageList");
        ...
        // KLICK ACTION
        $res[action] = array();
        $res[action][showName] = $useClass->lga("content","actionTab");
    ...
    function editContent_imageSliderSettings() {
        ...
        $res[slider] = array();
        $res[slider][showName] = $useClass->lga("content","sliderTab");
...
//        $checked = "";
//        if ($useClass->editContent[data][zoom]) $checked = " checked='checked'";
...
//        $addData = array();
//        $addData["text"] = "Höhe";    
//        $addData["input"] = "<input name='editContent[data][sliderHeight]' style='width:100px;' value='".$useClass->editContent[data][sliderHeight]."'>";
//        $res[slider][] = $addData;
