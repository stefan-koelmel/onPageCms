...
//
       ...
       if ($_POST[cancelSave]) { // abbrechen
        ...
        div_start("cmsImageData","float:left;width:".($frameWidth-$showSize-20)."px;");
        ...
        $leftWidth = 150;
        $rightWidth = $frameWidth - $leftWidth - 20 - $showSize;
        $standardHeight = 100;
        ...
        foreach($imageList as $fileName => $imageData) { // ($i=0;$i<count($imageList);$i++) {
            ...
            switch ($imageData[showType]) {
                ...
                case "file" :
                    ...
                    // echo ($imageId);
                    div_start("imageSingle","background-color:#eee;float:left;width:".($imageSize+0)."px;height:".($imageSize+$heightAdd)."px;border:1px solid #aaa;padding:5px;margin-right:5px;margin-bottom:5px;overflow:hidden;");
                    if ($imageData[type]) {
