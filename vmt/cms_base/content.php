...
//    }
    ...
    if (count($contentList) > 0 ) {
        ...
        for ($i=0;$i<count($contentList);$i++) {
            ...
            if ($contentShowLevel > $showLevel) {
                //  echo ("DONT Show because $showLevel <= show_Level = $contentShowLevel <br>");
                $show = 0;
            }
            
            $contentToLevel = $contentData[toLevel];
            
            if ($showLevel >= $contentToLevel AND $contentToLevel) {
                $show = 0;
            }
    ...
    //                            echo ("subPageNavi->show_index() - Not Session".subStr($subKey,strlen($subKey)-3)." <- $subKey <br>");
        ...
        if ($_SESSION[showLevel] > 4) {
            echo ("<div class='cmsContentNoData'>");



            switch ($type) {
                case "frame" : echo ("Kein Inhalt in diesem Rahmen"); break;
                case "page" : echo ("Kein Inhalt auf dieser Seite"); break;
                default :
...
function  cms_content_saveLayout($layoutData,$pageName) {
    ...
    $goPage = cms_page_goPage(); // $GLOBALS[pageData][name].".php";
