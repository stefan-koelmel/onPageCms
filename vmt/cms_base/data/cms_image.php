...
function cmsImage_upload_File($fileData,$imageFolder) {
...
//    }
    ...
    $name = cmsImage_checkFileName($name);
    ...
    if (is_array($imageInfo)) {
        ...
        if ($imgUploadWidth > 1000 OR $imgUploadHeight>1000) {
            cms_errorBox("Bild ist größer als die erlaubte Maximalgröße<br>Bild hat ein Auflösung von  $imgUploadWidth x $imgUploadHeight Pixel<br>Das Bild wurde nicht hochgeladen!");
    ...
    if ($existTargetFile) {
        $imageData = cmsImage_get(array("fileName"=>$name,"orgpath"=>"images/".$imageFolder));
        if (is_array($imageData)) { // file is in Database
            ...
            if (is_array($imageInfo)) {
                ...
                if ($imgUploadWidth != $imageData[width]) { echo ("update because with dirfent width<br>"); $update = 1;}
               // echo ("Width $imageInfo[0] database = $imageData[width] <br>");
                if ($imgUploadHeight != $imageData[height]) { echo ("update because with dirfent height<br>"); $update = 1;}
...
//         echo ("show = $showWidth x $showHeight <br>");
    ...
    switch ($vAlign) {
        ...
        case "middle" : $absVer = intVal($spaceVer / 2); $absVerBottom = intVal($spaceVer / 2);break;
        case "bottom" : $absVer = $spaceVer; break;
        default : $absVer = 0;
    }
    ...
    switch ($hAlign) {
        case "left"   : $absHor = 0; break;
        case "center" : $absHor = intVal($spaceHor / 2); break;
        case "right"  : $absHor = $spaceHor; break;
        default : $absHor = 0;
    }
   ...
   // if ($path[0] == "/") $path = substr($path,1);
    ...
    $imgStr .= "style='padding-top:".$absVer."px;padding-left:".$absHor."px;";
    if ($absVerBottom>0) {
        $imgStr .= "padding-bottom:".$absVerBottom."px;";
    }
    $imgStr .= "' width='".$showWidth."px' height='".$showHeight."px' />";
...
function cmsImage_ImageSmallExist($imageData,$showWidth,$showHeight,$imgWidth,$imgHeight,$off_X,$off_Y) {
   ...
   //  echo ("not exist ".$homePathServer.$folder.$newFileName." <br>");
...
//            echo ("$homePath $orgFile<br>");
//            echo ("$newFile <br>");
//            include('/usr/bin/imagemagick.class.php');
//
//
//            $image = Imagick($homePath.$orgFile);
//
//            // If 0 is provided as a width or height parameter,
//// aspect ratio is maintained
//            $image->thumbnailImage(100, 0);
//            $image->writeImagesFile ($newFile);
//
//            echo $image;
            //die();
...
function cmsImage_SelectList_getContent($folder) {
    ...
    foreach ($pathList as $folderName => $imageCount) {
        $divData[style] = "width:".$width."px;height:".$height."px;float:left;";
//        $divData[imageId] = "".$imageData[id];
//        $divData[folderName] = $folder.$folderName."/";
    ...
    if (!$result) {
        ...
        While ($imageData = mysql_fetch_assoc($result)) {
            ...
            if ($imgStr ) {
                ...
                $divData[style] = "width:".$width."px;height:".($height+20)."px;float:left;";
                $divData[imageId] = "".$imageData[id];
                $divData[imageSrc] = $imageSrc;
                ...
                if ($showName) $res.= "<div style='width:".$width."px;height:".$width."px;'>";
                if ($dragAble) $res.= "<div class='dragImage' id='id:$imageId|path:$imagePath' >";
                $res.= $imgStr;
                if ($dragAble) $res.= "</div>";
...
function cmsImage_listContent($imgList,$delimiter,$dataName,$width,$height,$imgWidth=100,$imgHeight=75,$showListData=array()){
    ...
    // $divName = "cmsImage_listList";
    $divData = array();
    $divData[style] = "width:".$width."px;background-color:#eee;visible:visible;overflow:auto;height:".$height."px;border-bottom;1px solid #fff;";
    if ($imageSortAble) $divData["class"].= " cmsImageSortList";
   //  if ($imageDeleteAble) $divData["class"].= " cmsImageDeleteList";
