<?php
 ...
 $server = 'db65.kundenserver.de'; // Datenbankserver
 $dbName = 'db69701053';  // Datenbank
 $name = 'dbo69701053';        // Name
 $pass = 'Wyatt';            // Kennwort
...
function getInfo_TableInfo($data,$debug) {
    ...
    $queryRow = "select * from `".$tableName."` LIMIT 0,10";
...
function getData_Tables($data,$debug) {
    ...
    $limit = "";
    if ($_GET[limitPage] AND $_GET[limitCount]) {
        $limitPage = $_GET[limitPage];
        $limitCount = $_GET[limitCount];
        $limitStart = ($limitPage-1) * $limitCount;
        $limit = "LIMIT $limitStart , $limitCount";
    }


    $query = "SELECT * FROM `$tableName` ".$filterQuery." ".$limit;
    $result = mysql_query($query);
    ...
    while ($line = mysql_fetch_assoc($result)) {
        ...
        foreach($line as $key=>$value) {
            if (is_string($value)) $value = iconv('ISO-8859-1', 'ISO-8859-15', $value);
            $newData[$key]=$value;
        }
        $res[] = $newData;
        $i++;
